...
- 將聊天與五項導覽遷移至 `designs/hub-portrait.pen`，保留兩份元件庫引用；底部隨父畫框寬度伸縮、導覽等寬排列、聊天正文自動換行並保留黑色外框。新增 320／375／430px 預覽與寬度檢查紀錄，未修改正式規範或全域 token。

- 將 Hub 底部元件庫拆為獨立的 `hub-nav.lib.pen`（5 個主元件）與 `hub-chat.lib.pen`（4 個主元件），各自提供維護文件、清單與預覽。原整合庫改由匯入兩份庫的 `hub-bottom-example.pen` 取代；既有使用端需依移轉說明更新引用，視覺及全域 token 不變。
...
- 修正聊天自適應：頻道、發話者與正文改為完整段落連續換行，移除固定第二行及右半欄正文；同步聊天庫、直向 Hub 主稿、底部示例與預覽，保留頻道色及 1.5px 黑色外框。舊使用端需移除舊文字子層覆寫，四個公開主元件 ID 不變。

- 修正 Hub 聊天庫遺漏的文字外框：核對原稿並替商會、私訊、世界的全部文字補上 1.5px 黑色外輪廓，以 7 個可編輯文字來源連動 168 個外框實例；保留訊息尺寸，重新載入並儲存底部組合示例，同步兩份預覽與維護說明。
